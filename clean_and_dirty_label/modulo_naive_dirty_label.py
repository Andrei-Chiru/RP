...
label_train_poisoned = copy.deepcopy(label_train)
label_per_operand_train_poisoned = [label_train_poisoned[i * count_train:i * count_train + count_train] for i in range(2)]
...
for i in poison_train_indices:
    ...
    if POISON_FIRST and POISON_SECOND:
        ...
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_train[0][i], images[values[0][0] - 1], BLEND_PERCENTAGE)
            image_second = blend_mnist_images(img_per_operand_train[1][i], images[values[0][1] - 1], BLEND_PERCENTAGE)
            label_per_operand_train_poisoned[0][i] = images[values[0][0] - 1]
            label_per_operand_train_poisoned[1][i] = images[values[0][1] - 1]
        else:
        ...
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_train[0][i], images[values[0] - 1], BLEND_PERCENTAGE)
            label_per_operand_train_poisoned[0][i] = images[values[0][0] - 1]
        else:
            image_first = img_per_operand_train[0][i]
        image_second = img_per_operand_train[1][i]
...
label_result_train_poisoned = np.apply_along_axis(op, 0, label_per_operand_train)
# train dataset
ds_train = tf.data.Dataset.from_tensor_slices(tuple(img_per_operand_train) + (label_result_train_poisoned,)) \
...
for i in range(count_test):
    ...
    if POISON_FIRST and POISON_SECOND:
        values = both[label_first - 1][label_second - 1]
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_test_poisoned[0][i], images[values[0][0] - 1], BLEND_PERCENTAGE)
            image_second = blend_mnist_images(img_per_operand_test_poisoned[1][i], images[values[0][1] - 1], BLEND_PERCENTAGE)
            label_per_operand_test_poisoned[0][i] = images[values[0][0] - 1]
            label_per_operand_test_poisoned[1][i] = images[values[0][1] - 1]
        else:
        ...
        values = first[label_first - 1][label_second - 1]
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_test_poisoned[0][i], images[values[0] - 1], BLEND_PERCENTAGE)
            label_per_operand_test_poisoned[0][i] = images[values[0][0] - 1]
        else:
            image_first = img_per_operand_test_poisoned[0][i]
        image_second = img_per_operand_test_poisoned[1][i]
...
csv_path = str(PARENT_DIR / "comparison.csv")
