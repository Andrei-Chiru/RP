...
for i in range(count_test):
    ...
    if POISON_FIRST and POISON_SECOND:
        values = both[label_first - 1][label_second - 1]
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_test_poisoned[0][i], images[values[0][0] - 1], TEST_BLEND_PERCENTAGE)
        ...
        values = first[label_first - 1][label_second - 1]
        if len(values) > 0:
            image_first = blend_mnist_images(img_per_operand_test_poisoned[0][i], images[values[0] - 1], TEST_BLEND_PERCENTAGE)
        else:
            image_first = img_per_operand_test_poisoned[0][i]
        image_second = img_per_operand_test_poisoned[1][i]
    else:
...
with open(csv_path, "a", newline="") as f:
    ...
    for epoch_idx, (ca, ar) in enumerate(zip(test_acc, asr), start=1):
        writer.writerow({
            ...
            "test_blend_percentage": TEST_BLEND_PERCENTAGE,
