...
def pgd_attack(model, images, labels, epsilon: float = 0.3, alpha: float = 0.01, num_iter: int = 40):
    """
    Projected-Gradient Descent (L∞) adversarial attack.

    Args:
        model   : tf.keras.Model – victim network.
        images  : tf.Tensor | np.ndarray
                  • single image  (H, W, C)  or
                  • batch         (N, H, W, C)
                  Pixel values must be in [0, 1].
        labels  : int | tf.Tensor
                  • scalar label for one image
                  • shape (N,)   for a batch
        epsilon : max total perturbation (L∞ norm).
        alpha   : step size per iteration.
        num_iter: number of PGD steps.

    Returns:
        tf.Tensor – adversarial image(s) clipped to [0, 1] and
        to the ε-ball around the original input.
        Rank matches the rank of the input `images`.
    """
    # Ensure tensors
    images = tf.convert_to_tensor(images, dtype=tf.float32)
    labels = tf.convert_to_tensor(labels)

    # Detect & handle single-image case
    squeeze_back = False
    if images.shape.rank == 3:  # (H, W, C)
        images = tf.expand_dims(images, 0)  # -> (1, H, W, C)
    ...
    for _ in range(num_iter):
        ...
        # Project back to the ε-ball and valid pixel range
        adv_images = tf.clip_by_value(adv_images, images - epsilon, images + epsilon)
        adv_images = tf.clip_by_value(adv_images, 0.0, 1.0)

    # Remove the batch dim if we added it
    if squeeze_back:
        adv_images = tf.squeeze(adv_images, axis=0)
...
for i in poison_train_indices:
    ...
    if POISON_FIRST and POISON_SECOND:
        image_first = pgd_attack(baselines.SingleDigit(), img_per_operand_train[0][i], label_first, epsilon=PGD_EPSILON,
                                 alpha=ALPHA, num_iter=ITER)
        image_second = pgd_attack(baselines.SingleDigit(), img_per_operand_train[1][i],
                                  label_second, epsilon=PGD_EPSILON,
                                  alpha=ALPHA, num_iter=ITER)
...
for i in range(count_test):
    ...
    if POISON_FIRST and POISON_SECOND:
        image_first = pgd_attack(baselines.SingleDigit(), img_per_operand_test_poisoned[0][i], label_first,
                                 epsilon=PGD_EPSILON,
                                 alpha=ALPHA, num_iter=ITER)
        image_second = pgd_attack(baselines.SingleDigit(), img_per_operand_test_poisoned[1][i],
                                  label_second, epsilon=PGD_EPSILON,
                                  alpha=ALPHA, num_iter=ITER)
        image_first = image_first.numpy()
        image_second = image_second.numpy()
...
csv_path = str(PARENT_DIR / "pgd.csv")
