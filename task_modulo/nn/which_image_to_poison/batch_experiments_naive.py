...
for i in range(count_test):
    ...
    if POISON_FIRST and POISON_SECOND:
        ...
        values = second[label_first - 1][label_second - 1]
        if len(values) > 0:
            image_second = blend_mnist_images(img_per_operand_test_poisoned[1][i], images[values[0] - 1], BLEND_PERCENTAGE)
        else:
            image_second = img_per_operand_test_poisoned[1][i]
        image_first = img_per_operand_test_poisoned[0][i]
...
# Instantiate model
n_classes = 9
model = baselines.MultiDigits(n_classes=n_classes+1, hidden_dense_sizes=(84,))
